...
class Transformers(Model):
    def __init__(self, vocab: Vocabulary, model_name: str, beam_size: int = 3, max_decoding_steps=100, **kwargs) -> None:
        ...
        if "t5" in model_name:
            self.model = T5Wrapper(vocab, model_name, max_decoding_steps=max_decoding_steps, beam_size=beam_size)
        elif "bart" in model_name:
            self.model = Bart(model_name, vocab, max_decoding_steps=max_decoding_steps, beam_size=beam_size)
        else:
            raise ValueError()
    ...
    def get_metrics(self, reset: bool = False) -> Dict[str, float]:
        return {
            "accuracy": self._metrics_accuracy.get_metric(reset=reset)
        }
